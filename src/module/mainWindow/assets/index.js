...
`;const o=document.getElementById("nas-shared-path");if(o){let s=!1,e=!1;document.addEventListener("keydown",n=>{n.key==="Tab"&&(s=!0)}),document.addEventListener("mousedown",()=>{s=!1}),document.addEventListener("focusin",n=>{n.target!==o&&(e=!1)}),o.addEventListener("focus",()=>{!s||e||(s=!1,e=!0,o.click())})}const{ipcRenderer:a}=window.require("electron");function c(s,e){const n=document.getElementById(s);n&&(n.value=e??"")}function r(s,e){const n=document.getElementById(s);n&&(n.checked=!!e,n.setAttribute("aria-checked",!!e))}a.on("mainWindow-msg",(s,e)=>{f.log("ui2 mainWindow-msg",e),e.action==="set-config"&&e.data?e.data.hasOwnProperty("nasURL")&&(c("nas-url",e.data.nasURL),c("nas-shared-path",e.data.sharedPath),e.data.hasOwnProperty("regProtocol")&&r("reg-protocol",e.data.regProtocol),e.data.hasOwnProperty("showSpeedWindow")&&r("show-speed-window",e.data.showSpeedWindow)):e.action==="confirm-shared-path"&&e.data&&e.data.filePaths&&c("nas-shared-path",e.data.filePaths)});const u=document.getElementById("confirm-config");u&&u.addEventListener("click",()=>{a.send("mainWindow-msg",{action:"confirm-config",data:{nasURL:document.getElementById("nas-url")?document.getElementById("nas-url").value:"",regProtocol:document.getElementById("reg-protocol")?document.getElementById("reg-protocol").checked:!1,sharedPath:document.getElementById("nas-shared-path")?document.getElementById("nas-shared-path").value:"",showSpeedWindow:document.getElementById("show-speed-window")?document.getElementById("show-speed-window").checked:!1}})});o&&o.addEventListener("click",()=>{a.send("mainWindow-msg",{action:"confirm-shared-path",data:{nasURL:document.getElementById("nas-url")?document.getElementById("nas-url").value:""}})});const p=document.getElementById("check-update");p&&p.addEventListener("click",()=>{a.send("mainWindow-msg",{action:"check-update"})});
